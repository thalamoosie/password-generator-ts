...
const leetSpeak: { [key: string]: string[] } = {
  a: ["4", "@"],
  s: ["5", "$"],
  e: ["3", "e"],
  t: ["7", "t"],
...
const substituteWords = function (arr: string[], key: string) {
  for (let i = 0; i < arr.length; i++) {
    arr[i] = arr[i].replace(
      new RegExp(`[${key}]`, "gi"),
      (char: string) =>
        leetSpeak[char]?.[randomArrIndex(leetSpeak[char])] ?? char
    );
    //pass
  }
  return arr;
...
const generatePassphrase = function (
  ...
  try {
    ...
    const phraseArray: string[] = Array.from(
      searchLetters,
    ...
    if (charSwap) {
      ...
      const charKey: string = Object.keys(leetSpeak).join("");
      substituteWords(phraseArray, charKey);
    }
    // Capitalize 1 random word in all caps if selected:
    if (capitalize) {
      const randomIndex: number = randomArrIndex(phraseArray);
      const capWord: string = phraseArray[randomIndex].toUpperCase();
